...
import { Container, Volume } from '../types/docker'
import '../styles/VolumeConnection.css'
...
export const VolumeConnection: React.FC<VolumeConnectionProps> = ({ 
  ...
  return (
    <div 
      className={`volume-connection-container ${connectionClass}`}
      data-container-id={container.id}
      data-volume-id={volume.id}
    >
      <svg className="connection-path" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
        <path 
          d={path}
          stroke="#1c7ed6" 
          strokeWidth="3" 
      ...
      {isConnecting && (
        <div className="pulse-dot" style={{ left: '50%', bottom: '5px' }}></div>
      )}
    </div>
  )
} 