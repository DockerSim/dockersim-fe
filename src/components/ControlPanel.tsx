...
const ControlPanel: React.FC<ControlPanelProps> = ({ isCollapsed = false, showToast }) => {
  ...
  const handleContainerAction = (containerName: string, action: 'start' | 'stop' | 'pause' | 'rm') => {
  ...
  const handleResourceCreationConfirm = (data: ResourceCreationData) => {
    if (data.image) { // Container Creation
      ...
      if(data.networkId) command += ` --network ${data.networkId}`;
      command += ` ${data.image}`;
                  ...
                  {(containers || []).map(container => (
                    <div key={container.id} className="resource-item">
                      ...
                      <div className="resource-actions">
                        {container.status === 'stopped' && <button className="action-btn start" onClick={() => handleContainerAction(container.name, 'start')}>▶️</button>}
                        {container.status === 'running' && <button className="action-btn stop" onClick={() => handleContainerAction(container.name, 'stop')}>⏹️</button>}
                        {container.status === 'running' && <button className="action-btn pause" onClick={() => handleContainerAction(container.name, 'pause')}>⏸️</button>}
                        <button className="action-btn remove" onClick={() => handleContainerAction(container.name, 'rm')}>🗑️</button>
                      </div>
                    </div>
                  ))}
